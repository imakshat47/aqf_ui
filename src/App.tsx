...
// ── CONFIG ────────────────────────────────────────────────────────────────────
// Change to your FastAPI server URL when running locally:
//   const API_BASE = "http://localhost:8000";
const API_BASE: string | null = "https://aqf-v2.onrender.com/";
...
export interface FormField {
  ...
  full_label: string;
  dv_type: string;
  suggestion_mode: "categorical" | "boolean" | "numeric" | "date" | "none";
...
export type CritMap = Record<string, CriterionState>;
...
// ── DEMO DATA (mirrors exact backend output for the real dataset) ─────────────

const DEMO_LOAD: LoadResponse = {
  ...
  form: {
    criteria_groups: [
      ...
      { group_label: "Problem / Diagnosis", subgroups: [
        { label: "(no cluster)", fields: [
...
const DEMO_RESULT: QueryResult = {
  ...
  output_labels: ["HCPA → General data → issue date", "Problem/Diagnosis → Problem"],
};

// ── API LAYER ─────────────────────────────────────────────────────────────────

async function apiLoad(dir: string): Promise<LoadResponse> {
...
interface QueryPayload {
  data_dir: string;
  criteria: Array<{ field_key: string; operator: string; value: string | string[] | null }>;
  output_fields: Array<{ field_key: string; name: string; dv_type: string }>;
  sort: SortState | null;
  advanced: AdvancedState;
}

async function apiQuery(payload: QueryPayload): Promise<QueryResult> {
  if (!API_BASE) {
...
const DV_META: Record<string, DvMeta> = {
...
const dvm = (t: string): DvMeta =>
  DV_META[t] ?? { bg: "#f1f5f9", text: "#475569", tag: t?.slice(3, 8) ?? "?" };
...
const DVTag: React.FC<{ type: string }> = ({ type }) => {
  const m = dvm(type);
  return (
    <span style={{ fontSize: 9, fontWeight: 800, letterSpacing: "0.07em", padding: "1px 5px", borderRadius: 4, background: m.bg, color: m.text }}>
      {m.tag}
    </span>
  );
};
...
interface ChipProps {
  ...
  label: string;
  onX?: () => void;
  hue?: "teal" | "violet" | "amber";
}

const Chip: React.FC<ChipProps> = ({ pre, label, onX, hue = "teal" }) => {
  const s =
    hue === "violet" ? { bg: T.violetBg, text: T.violet,   bd: T.violetBorder } :
  ...
  return (
    <span style={{ display: "inline-flex", alignItems: "center", gap: 5, padding: "3px 10px", borderRadius: 20, fontSize: 12, fontWeight: 500, background: s.bg, color: s.text, border: `1px solid ${s.bd}` }}>
      ...
      {label}
      {onX && (
        <button onClick={onX} style={{ background: "none", border: "none", cursor: "pointer", padding: 0, color: "inherit", opacity: 0.5, lineHeight: 1, fontSize: 13 }}>×</button>
      )}
    </span>
  );
};
...
const Panel: React.FC<PanelProps> = ({ title, badge, icon, open, onToggle, accent = false, children }) => (
  <div style={{ border: `1px solid ${accent ? T.tealBorder : T.border}`, borderRadius: 12, background: T.white, overflow: "hidden" }}>
    ...
    >
      <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
        ...
        {badge && (
          <span style={{ fontSize: 11, fontWeight: 600, background: accent ? T.tealBg : "#f1f5f9", color: accent ? T.tealDark : T.muted, padding: "2px 8px", borderRadius: 20, border: `1px solid ${accent ? T.tealBorder : T.border}` }}>
            {badge}
          </span>
        )}
      </div>
    ...
    {open && (
      <div style={{ borderTop: `1px solid ${T.border}`, padding: 16, background: "#fafbfc" }}>
        {children}
      </div>
    )}
  </div>
...
interface CritRowProps {
  field: FormField;
  value: CriterionState;
  onChange: (v: CriterionState) => void;
  onRemove: () => void;
}

const CritRow: React.FC<CritRowProps> = ({ field, value, onChange, onRemove }) => {
  ...
  const inp: React.CSSProperties = { fontFamily: "inherit", fontSize: 12, border: `1px solid ${T.border}`, borderRadius: 8, padding: "5px 9px", background: T.white, color: T.ink, outline: "none", boxSizing: "border-box" };
  const upd = (patch: Partial<CriterionState>) => onChange({ operator, rawValue, rangeHi, ...patch });
  ...
  return (
    <div style={{ background: T.tealBg, border: `1px solid ${T.tealBorder}`, borderRadius: 10, padding: "10px 12px" }}>
      ...
      <div style={{ display: "flex", gap: 7, flexWrap: "wrap" }}>
        <select value={operator} onChange={(e: ChangeEvent<HTMLSelectElement>) => upd({ operator: e.target.value })} style={{ ...inp, flexShrink: 0 }}>
          {field.operators.map((o) => <option key={o.op} value={o.op}>{o.phrase}</option>)}
        </select>

        {!isNull && !isBetween && isCat && (
          <select value={rawValue} onChange={(e: ChangeEvent<HTMLSelectElement>) => upd({ rawValue: e.target.value })} style={{ ...inp, flex: 1 }}>
        ...
        {!isNull && !isBetween && !isCat && (
          <input
            type={field.suggestion_mode === "numeric" ? "number" : "text"}
            placeholder={field.suggested_values[0] ?? "value"}
            value={rawValue}
            onChange={(e: ChangeEvent<HTMLInputElement>) => upd({ rawValue: e.target.value })}
            style={{ ...inp, flex: 1 }}
          />
        )}
        ...
        {isBetween && (
          <>
            <input type="text" placeholder="from" value={rawValue} onChange={(e: ChangeEvent<HTMLInputElement>) => upd({ rawValue: e.target.value })} style={{ ...inp, width: 120 }} />
            <span style={{ alignSelf: "center", fontSize: 11, color: T.muted }}>→</span>
            <input type="text" placeholder="to" value={rangeHi} onChange={(e: ChangeEvent<HTMLInputElement>) => upd({ rangeHi: e.target.value })} style={{ ...inp, width: 120 }} />
          </>
        )}
...
const RCard: React.FC<RCardProps> = ({ row, labels, index }) => {
  ...
  const short   = (l: string) => l.split("→").pop()?.trim() ?? l;

  return (
    <div style={{ background: T.white, border: `1px solid ${T.border}`, borderRadius: 12, padding: 14 }}>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 10 }}>
        <span style={{ fontSize: 10, fontFamily: "monospace", color: "#94a3b8" }}>{String(row._file ?? `record #${index + 1}`)}</span>
      ...
      {exp && (
        <div style={{ marginTop: 8, display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(140px, 1fr))", gap: "8px 14px", borderTop: `1px solid ${T.border}`, paddingTop: 8 }}>
          {other.map((k) => (
            <div key={k}>
...
const AQFv2: React.FC = () => {
  const [dataDir,    setDataDir]    = useState<string>("data");
  const [pendingDir, setPendingDir] = useState<string>("data");  // buffer — does NOT trigger load
  ...
  const [pFilters,  setPFilters]  = useState<boolean>(true);
  const [pOutput,   setPOutput]   = useState<boolean>(false);
  const [pMore,     setPMore]     = useState<boolean>(false);
  const [pFunnel,   setPFunnel]   = useState<boolean>(false);
  const [pExplain,  setPExplain]  = useState<boolean>(false);
  const [groupOpen, setGroupOpen] = useState<Record<string, boolean>>({});

  // query state
  const [crit,    setCrit]    = useState<CritMap>({});
  const [outKeys, setOutKeys] = useState<Set<string>>(new Set());
  const [sort,    setSort]    = useState<SortState>({ field_key: "", direction: "desc" });
  const [adv,     setAdv]     = useState<AdvancedState>({ occurrence_semantics: "ALL", include_unknown: false, slice_size: 1000, result_limit: 100 });
  ...
  // ── Load schema whenever committed dataDir changes ──────────────────────────
  useEffect(() => {
    setLoading(true);
    apiLoad(dataDir)
      .then((d) => {
  ...
  // ── Derived helpers ─────────────────────────────────────────────────────────
  const allFields: FormField[] = formDef
    ? formDef.criteria_groups.flatMap((cg) => cg.subgroups.flatMap((sg) => sg.fields))
    : [];
  const byKey: Record<string, FormField> = Object.fromEntries(allFields.map((f) => [f.field_key, f]));
  const activeCnt = Object.keys(crit).length;
  const outFields: OutputField[] = formDef?.output_fields ?? [];
  const activeOutLabels: string[] = outFields.filter((f) => outKeys.has(f.field_key)).map((f) => f.label);

  const addCrit = (f: FormField) => {
    if (crit[f.field_key]) return;
    setCrit((p) => ({ ...p, [f.field_key]: { operator: f.operators[0]?.op ?? "=", rawValue: "", rangeHi: "" } }));
  };
  const rmCrit  = (fk: string) => setCrit((p) => { const n = { ...p }; delete n[fk]; return n; });
  const updCrit = (fk: string, v: CriterionState) => setCrit((p) => ({ ...p, [fk]: v }));
  const toggleOut = (fk: string) =>
    setOutKeys((p) => { const n = new Set(p); n.has(fk) ? n.delete(fk) : n.add(fk); return n; });
  ...
  const summary = (): string => {
    const parts = Object.entries(crit).map(([fk, v]) => {
      ...
      if (["is_known", "is_unknown"].includes(v.operator)) return `${f.label} ${op?.phrase ?? v.operator}`;
      if (!v.rawValue) return `${f.label} ${op?.phrase ?? v.operator} …`;
      if (v.operator === "between") return `${f.label} between ${v.rawValue} and ${v.rangeHi || "…"}`;
      return `${f.label} ${op?.phrase ?? v.operator} "${v.rawValue}"`;
  ...
  const runQuery = async (): Promise<void> => {
    ...
    try {
      const critList = Object.entries(crit).map(([fk, v]) => ({
        field_key: fk,
        operator: v.operator,
        value: (["is_known", "is_unknown"].includes(v.operator)
          ? null
          : v.operator === "between"
            ? [v.rawValue, v.rangeHi]
            : v.rawValue || null) as string | string[] | null,
      ...
      const payload: QueryPayload = {
        ...
        output_fields: outFields
          .filter((f) => outKeys.has(f.field_key))
          .map((f) => ({ field_key: f.field_key, name: f.label, dv_type: f.dv_type })),
  ...
  const inp = (extra: React.CSSProperties = {}): React.CSSProperties => ({
    fontFamily: "inherit", fontSize: 12, border: `1px solid ${T.border}`,
    borderRadius: 8, padding: "5px 9px", background: T.white, color: T.ink,
    outline: "none", ...extra,
  });
  ...
  if (loadErr) return (
    <div style={{ minHeight: "100vh", background: T.bg, display: "flex", alignItems: "center", justifyContent: "center", fontFamily: "system-ui, sans-serif" }}>
      <div style={{ background: T.white, border: `1px solid ${T.redBorder}`, borderRadius: 12, padding: 24, textAlign: "center", maxWidth: 380 }}>
        ...
        <button onClick={() => setDataDir(pendingDir)} style={{ marginTop: 12, fontFamily: "inherit", fontSize: 12, padding: "7px 18px", borderRadius: 8, border: `1px solid ${T.teal}`, background: T.teal, color: T.white, cursor: "pointer" }}>
          Retry
        </button>
      </div>
    </div>
    ...
    <div style={{ fontFamily: "'DM Sans', system-ui, sans-serif", minHeight: "100vh", background: T.bg, width: "100%" }}>
            ...
            {meta && (
              <>
                ...
                <span style={{ fontSize: 11, color: T.muted }}>
                  · <b style={{ color: T.ink }}>{meta.stats.files}</b> files
                  · <b style={{ color: T.ink }}>{meta.stats.fields}</b> fields
                </span>
              </>
            )}
          </div>

          {/* Data dir input + Apply button */}
          <div style={{ display: "flex", gap: 6 }}>
            <input
              value={pendingDir}
              onChange={(e: ChangeEvent<HTMLInputElement>) => setPendingDir(e.target.value)}
              onKeyDown={(e: KeyboardEvent<HTMLInputElement>) => { if (e.key === "Enter") setDataDir(pendingDir); }}
      ...
      <div style={{ background: T.tealDark, padding: "8px 20px", position: "sticky", top: 0, zIndex: 100 }}>
        <div style={{ maxWidth: 880, margin: "0 auto", display: "flex", alignItems: "center", gap: 10 }}>
          ...
          {activeCnt > 0 && (
            <span style={{ fontSize: 11, background: "rgba(255,255,255,.15)", color: "#ccfbf1", padding: "2px 10px", borderRadius: 20, flexShrink: 0 }}>
              {activeCnt} filter{activeCnt !== 1 ? "s" : ""}
            </span>
          )}
        </div>
      </div>
      ...
      <div style={{ maxWidth: 880, margin: "0 auto", padding: "16px 20px", display: "flex", flexDirection: "column", gap: 10 }}>
        ...
        {(activeCnt > 0 || outKeys.size > 0) && (
          <div style={{ display: "flex", flexWrap: "wrap", gap: 5, alignItems: "center", padding: "8px 12px", background: T.white, borderRadius: 10, border: `1px solid ${T.border}` }}>
            ...
            {Object.entries(crit).map(([fk, v]) => {
              ...
              if (!f) return null;
              const op    = f.operators.find((o) => o.op === v.operator);
              const isNull = ["is_known", "is_unknown"].includes(v.operator);
              const val   = isNull ? "" : v.operator === "between" ? ` ${v.rawValue}–${v.rangeHi}` : v.rawValue ? ` "${v.rawValue}"` : " …";
              return <Chip key={fk} pre={f.label} label={`${op?.phrase ?? v.operator}${val}`} hue="teal" onX={() => rmCrit(fk)} />;
            })}
            {outFields.filter((f) => outKeys.has(f.field_key)).slice(0, 3).map((f) => (
              <Chip key={f.field_key} pre="show" label={f.label.split("→").pop()?.trim() ?? f.label} hue="violet" onX={() => toggleOut(f.field_key)} />
            ))}
            ...
            {formDef?.criteria_groups.map((cg) => {
              ...
              return (
                <div key={cg.group_label} style={{ border: `1px solid ${T.border}`, borderRadius: 10, background: T.white, overflow: "hidden" }}>
                  <button
                    onClick={() => setGroupOpen((p) => ({ ...p, [cg.group_label]: !p[cg.group_label] }))}
                  ...
                  {isOpen && (
                    <div style={{ borderTop: `1px solid ${T.border}` }}>
                      {cg.subgroups.map((sg) => {
                        const sflds = sg.fields.filter((f) =>
                          !search || f.label.toLowerCase().includes(search.toLowerCase()) || f.full_label.toLowerCase().includes(search.toLowerCase())
                        );
                        ...
                        return (
                          <div key={sg.label}>
                            {sg.label !== "(no cluster)" && (
                              <div style={{ fontSize: 10, fontWeight: 700, color: "#94a3b8", textTransform: "uppercase", letterSpacing: ".06em", padding: "5px 14px 2px" }}>
                                {sg.label}
                              </div>
                            )}
                            {sflds.map((field) => {
                              ...
                              return (
                                ...
                                >
                                  <div style={{ display: "flex", alignItems: "center", gap: 7, overflow: "hidden" }}>
                                    ...
                                    {field.suggested_values[0] && !active && (
                                      <span style={{ fontSize: 10, color: "#94a3b8", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>e.g. {field.suggested_values[0]}</span>
                                    )}
                                  </div>
                                  <button
                                    onClick={() => active ? rmCrit(field.field_key) : addCrit(field)}
        ...
        <Panel title="Results to show" badge={`${outKeys.size} fields`} icon="👁" open={pOutput} onToggle={() => setPOutput((v) => !v)}>
          <div style={{ display: "flex", flexWrap: "wrap", gap: 6, marginBottom: 12 }}>
            {outFields.map((f) => {
              ...
              const name = f.label.split("→").pop()?.trim() ?? f.label;
              return (
          ...
          <div style={{ borderTop: `1px solid ${T.border}`, paddingTop: 10, display: "flex", gap: 7, alignItems: "center" }}>
            ...
            <select
              ...
              onChange={(e: ChangeEvent<HTMLSelectElement>) => setSort((p) => ({ ...p, field_key: e.target.value }))}
              style={inp({ flex: 1 } as React.CSSProperties)}
            >
              <option value="">— none —</option>
              {outFields.filter((f) => outKeys.has(f.field_key)).map((f) => (
                <option key={f.field_key} value={f.field_key}>{f.label.split("→").pop()?.trim()}</option>
              ))}
            </select>
        ...
        <Panel title="More options" icon="⚙" open={pMore} onToggle={() => setPMore((v) => !v)}>
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
            ...
            <div>
              ...
              <input type="number" value={adv.slice_size} onChange={(e: ChangeEvent<HTMLInputElement>) => setAdv((p) => ({ ...p, slice_size: Number(e.target.value) }))} style={inp({ width: "100%" })} />
            </div>
            <div>
              <div style={{ fontSize: 11, color: T.muted, marginBottom: 5 }}>Result limit</div>
              <input type="number" value={adv.result_limit} onChange={(e: ChangeEvent<HTMLInputElement>) => setAdv((p) => ({ ...p, result_limit: Number(e.target.value) }))} style={inp({ width: "100%" })} />
            </div>
          </div>
        </Panel>
        ...
        {qErr && (
          <div style={{ background: "#fef2f2", border: `1px solid ${T.redBorder}`, borderRadius: 10, padding: "9px 13px", fontSize: 12, color: T.red }}>{qErr}</div>
        )}
        ...
        {result && (
          <div>
            <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 10 }}>
              ...
              <div style={{ display: "flex", gap: 6 }}>
                {([["funnel", "↓ Funnel", pFunnel, setPFunnel], ["explain", "✦ Explain", pExplain, setPExplain]] as const).map(([k, lbl, open, setOpen]) => (
                  <button key={k} onClick={() => (setOpen as React.Dispatch<React.SetStateAction<boolean>>)((v) => !v)}
            ...
            {pExplain && (
              <div style={{ background: T.white, border: `1px solid ${T.border}`, borderRadius: 12, padding: 14, marginBottom: 10 }}>
                ...
                {activeCnt === 0 && (
                  <div style={{ background: T.amberBg, border: `1px solid ${T.amberBorder}`, borderRadius: 8, padding: "8px 12px", fontSize: 12, color: T.amber }}>
                    No filters active — all records matched.
                  </div>
                )}
                {Object.entries(crit).map(([fk, v]) => {
                  const f = byKey[fk];
                ...
                <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 12 }}>
                  <thead>
                    <tr style={{ background: "#f8fafc", borderBottom: `1px solid ${T.border}` }}>
                      {activeOutLabels.map((l) => (
                        <th key={l} style={{ textAlign: "left", padding: "9px 13px", color: T.muted, fontWeight: 600, fontSize: 10, textTransform: "uppercase", letterSpacing: ".05em", whiteSpace: "nowrap" }}>
                          {l.split("→").pop()?.trim()}
                        </th>
                      ))}
                    </tr>
                  ...
                  <tbody>
                    {result.rows.map((row, i) => (
                      <tr key={i} style={{ borderBottom: `1px solid #f1f5f9`, background: i % 2 === 0 ? T.white : "#fafbfc" }}>
                        {activeOutLabels.map((l) => (
                          <td key={l} style={{ padding: "7px 13px", color: T.slate, maxWidth: 220, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>
                            {row[l] != null ? String(row[l]) : <span style={{ color: "#94a3b8" }}>—</span>}
                          </td>
                        ))}
                      </tr>
                    ))}
          ...
          <div style={{ textAlign: "center", padding: "40px 0" }}>
            <div style={{ width: 44, height: 44, borderRadius: 12, background: T.tealBg, border: `1px solid ${T.tealBorder}`, margin: "0 auto 12px", display: "flex", alignItems: "center", justifyContent: "center" }}>
              <svg width="20" height="20" viewBox="0 0 20 20" fill="none">
                <circle cx="9" cy="9" r="6.5" stroke={T.teal} strokeWidth="1.8" />
                <path d="M14.5 14.5l4 4" stroke={T.teal} strokeWidth="1.8" strokeLinecap="round" />
              </svg>
            </div>
